...
export class AdminService {
  ...
  async getAllCategories(isActive) {
    ...
    return convertBigInt(categories);
  }
  ...
  async getCategoryById(id) {
    ...
    return convertBigInt(category);
  }

  async createCategory(data) {
    const { name, description, isActive = true, adminId } = data;

    // Check if category already exists
    const existing = await this.prisma.donationCategory.findUnique({
      where: { name }
    });
    ...
    const category = await this.prisma.donationCategory.create({
      data: {
        name,
        description,
        isActive
  ...
  async updateCategory(id, data) {
    const { name, description, isActive, adminId } = data;
    ...
    if (name && name !== existing.name) {
      const nameConflict = await this.prisma.donationCategory.findUnique({
        where: { name }
      });
    ...
    const category = await this.prisma.donationCategory.update({
      where: { id },
      data: {
        ...(name && { name }),
  ...
  async deleteCategory(id, adminId) {
    ...
    if (category._count.donations > 0) {
      throw new Error(
        `Cannot delete category with ${category._count.donations} associated donations. ` +
        'Please deactivate it instead or reassign the donations first.'
      );
    }

    await this.prisma.donationCategory.delete({
      where: { id }
    });
  ...
  async toggleCategoryStatus(id, adminId) {
    const category = await this.prisma.donationCategory.findUnique({
      where: { id }
    });
    ...
    const updated = await this.prisma.donationCategory.update({
      where: { id },
      data: {
    ...
    await createAuditLog({
      ...
      entityType: 'CATEGORY',
      entityId: id,
      description: `${updated.isActive ? 'Activated' : 'Deactivated'} category: ${category.name}`,
      metadata: { 
        categoryName: category.name,
