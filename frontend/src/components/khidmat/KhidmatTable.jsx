...
//
// FIX: Dropdowns were being clipped by the overflow-x-auto
// container. Solution: render all dropdown menus through
// ReactDOM.createPortal into document.body so they are
// completely outside the overflow stacking context.
// Position is calculated from the trigger button's
// getBoundingClientRect() so they appear in the right place.
// ============================================================
...
// ─────────────────────────────────────────────
// usePortalDropdown
// Tracks trigger button position and returns
// style for the portalled menu so it appears
// directly below the trigger button.
// ─────────────────────────────────────────────
...
const SkeletonRow = () => (
  <tr className="border-b border-slate-100 animate-pulse">
    {[40, 120, 90, 90, 80, 90, 60, 60].map((w, i) => (
...
// ─────────────────────────────────────────────
// StatusDropdown — portalled to body
// ─────────────────────────────────────────────
    ...
    <div className="relative inline-block">
      ...
      {open && !isUpdating && ReactDOM.createPortal(
        <>
          ...
          <div
            style={style}
            className="bg-white border border-slate-200 rounded-xl shadow-2xl overflow-hidden min-w-[148px]"
          >
            {STATUS_OPTS.map(opt => {
              const Icon      = opt.icon
              ...
              return (
                <button
                  key={opt.value}
                  onClick={() => {
                    if (!isCurrent) quickUpdateStatus(recordId, opt.value)
                    setOpen(false)
                  }}
                  className={`
                    w-full flex items-center gap-2 px-3 py-2.5 text-xs font-medium transition-colors
                    ${isCurrent
                      ? `${opt.colors.bg} ${opt.colors.text} cursor-default`
...
const WhatsAppButton = ({ record }) => {
  ...
  const alreadySent = record.whatsappSent

  return (
    <button
      onClick={() => sendWhatsApp(record.id)}
      disabled={isSending}
      title={alreadySent ? 'Resend WhatsApp message' : 'Send WhatsApp message'}
      className={`
...
// ─────────────────────────────────────────────
// RowActions menu — portalled to body
// Menu opens to the LEFT of the trigger so it
// never overflows the right edge of the table.
// ─────────────────────────────────────────────
const RowActions = ({ record }) => {
  ...
  // For actions menu we want it anchored to the RIGHT edge of the trigger
    ...
    <div className="relative inline-block">
      ...
      {open && ReactDOM.createPortal(
        <>
          ...
          <div
            style={menuStyle}
            className="bg-white border border-slate-200 rounded-xl shadow-2xl overflow-hidden min-w-[162px]"
          >
            <button
              onClick={() => { openEditForm(record); setOpen(false) }}
              className="w-full flex items-center gap-2.5 px-3.5 py-2.5 text-sm text-slate-600 hover:bg-slate-50 transition-colors"
            >
              <Pencil size={14} /> Edit
            </button>
            <button
              onClick={() => { downloadReceipt(record.id, record.name); setOpen(false) }}
              className="w-full flex items-center gap-2.5 px-3.5 py-2.5 text-sm text-slate-600 hover:bg-slate-50 transition-colors"
            >
  ...
  return (
    <div className="flex flex-col sm:flex-row items-center justify-between gap-3 px-4 py-3 border-t border-slate-100 bg-slate-50/60">
      ...
      <div className="flex items-center gap-1">
        ...
        {pageNums.map(n => (
          <PageBtn key={n} label={n} active={n === page} onClick={() => go(n)} />
        ))}
        {page < pages - 2 && (
          <><span className="px-1 text-slate-400 text-xs">…</span>
          <PageBtn label={pages} onClick={() => go(pages)} /></>
        )}
...
const PageBtn = ({ label, icon, active, onClick, disabled }) => (
  <button
    ...
    disabled={disabled}
    className={`
      min-w-[32px] h-8 rounded-lg px-2 text-xs font-medium transition-all flex items-center justify-center
...
const KhidmatTable = () => {
  ...
  return (
    <div>
      ...
      {/* ── DESKTOP TABLE ───────────────────── */}
      {/* NOTE: overflow-x-auto is kept for horizontal scroll on small
          screens, but dropdowns escape it via ReactDOM.createPortal  */}
      <div className="hidden md:block overflow-x-auto">
        <table className="w-full text-sm border-collapse">
          <thead>
            <tr className="border-b border-slate-200 bg-slate-50/80">
              {['Date', 'Name', 'Phone', 'Category', 'Amount', 'Status', 'WhatsApp', 'Actions'].map(h => (
                <th
                  key={h}
              ...
              : records.map(record => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3.5">
                      ...
                      {record.address && (
                        <div className="text-xs text-slate-400 mt-0.5 truncate max-w-[140px]">{record.address}</div>
                      )}
                    </td>
                    ...
                    <td className="px-4 py-3.5 font-semibold text-slate-800 whitespace-nowrap">
                      Rs {parseFloat(record.amount).toLocaleString('en-IN', { minimumFractionDigits: 0 })}
                    </td>
